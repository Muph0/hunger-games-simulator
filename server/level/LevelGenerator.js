...
LevelGenerator.Generate = function(seed, biome_count)
{
    var width = 50;
    var height = 25;
    ...
    for (var i = 0; i < width * height; i++)
    {
        ...
        // register the tile in the biome
        arena.Biomes[owner].TilesOwned.push(i);
        // let the biome decide on the visuals of the tile
        arena.Tiles[i] = arena.Biomes[owner].GenerateTile(rnd);
    }

    // plant special tiles

    // populate all tiles with entities and items
    for (var t = 0; t < width * height; t++)
    {
        for (entity_type of GameEntity.Types)
        {
            entity_type.Generator(rnd, tile);
        }

        for (item_type of Item.Types)
        {
            item_type.Generate(rnd, tile)
        }
    }
